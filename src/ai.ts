...
function getOpenRouterTools(modelId: string, tools?: any[]) {
  const openRouterTools = (tools || []).filter(
    tool => tool?.type === 'function' || tool?.type?.startsWith?.('openrouter:')
  );
  ...
  const missingServerTools = OPENROUTER_DEEPSEEK_SERVER_TOOLS.filter(
    tool => !existingToolTypes.has(tool.type)
  );
...
export async function generateContentWithFallback(
  ...
  if (preferredModels && preferredModels.length > 0) {
    ...
    modelsToTry = [...preferredModels, ...FALLBACK_MODELS.filter(m => !preferredIds.has(m.id))];
  } else {
    // Standard behavior: current conversational model first
    modelsToTry = [currentModel, ...FALLBACK_MODELS.filter(m => m.id !== currentModel.id)];
  }
