...
!effort low|medium|high|xhigh  reasoning do Grok 4.5/4.6/4.20-reasoning (não manda no *-non-reasoning)
...
Ordem aproximada no tick: **mood drift** (18%/tick, mínimo 3h) → **relationship / update_temperature** (25%/tick, mínimo 6h, mesmo se Alice estiver ausente) → WYWG (sozinha ≥ 3h) → offscreen → double-text → diário (≥ 18h) → sonho (noite, ≥ 14h). Mood e relationship rodam **antes** dos `return` de WYWG/double-text. Cada um tem chance + cooldown (`last_diary_at`, `last_dream_at`, `last_wywg_at`, `last_reach_at`; mood/rel usam `last_updated` no JSON).
